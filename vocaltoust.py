...
# --------------------------------------------------------
def print_usage():
    print("""Usage: python vocaltoust.py [--audio <audio_file>] [--lyrics path_or_url] [--no-gentle] [--model base] [--out out.ust]
Options:
  --audio : Path to the audio file (wav/mp3)
...
def cli_main(argv):
    ...
    while i < len(argv):
        a = argv[i]
        if a == "--audio" and i+1 < len(argv):
            audio = argv[i+1]; i += 2; continue
        ...
        # Backward compatibility: if no --audio, assume first arg is audio
        if audio is None and not a.startswith("--"):
            audio = a; i += 1; continue
        i += 1
